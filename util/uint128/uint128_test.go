...
func TestMod(t *testing.T) {
	...
	for i := 0; i < 100000; i++ {
		a := []uint32{rand.Uint32(), rand.Uint32(), rand.Uint32(), rand.Uint32()}
		n := rand.Int63n(int64(2)^21-1) + 1
		au := makeUint128(a)
		abig := makeBigInt(a)
		nbig := new(big.Int).SetUint64(uint64(n))
		resu := au.Mod(uint64(n))
		modbig.Mod(abig, nbig)
		//compare abig, resu
		resubig := new(big.Int).SetUint64(resu)
		if modbig.Cmp(resubig) != 0 {
			t.Errorf("Mod %x by %x (%x %x mod %x), expected %x got %x",
				abig, nbig, au.H, au.L, n, modbig, resubig)
		}
	}
}
